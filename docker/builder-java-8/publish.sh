...
export REPO_URL="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/jenkins/builder"
export JENKINS_TAG=`date +%Y-%m-%d_%H-%M`

docker build -t jenkins-builder .
docker tag jenkins-builder ${REPO_URL}:${JENKINS_TAG}
