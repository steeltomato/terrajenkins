...
export REPO_URL="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/jenkins/builder-docker"
export JENKINS_TAG=`date +%Y-%m-%d_%H-%M`

docker build -t jenkins-builder-docker .
docker tag jenkins-builder-docker ${REPO_URL}:${JENKINS_TAG}
